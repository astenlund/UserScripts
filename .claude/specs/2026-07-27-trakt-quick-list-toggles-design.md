...
A small script-drawn toast (fixed position, auto-dismiss after a few seconds, styled for both themes via the existing `tff-light` root class): the write-failure headline `Couldn't add "<title>" to <list>` / `Couldn't remove "<title>" from <list>`, suffixed with `(HTTP <status>)` when, and only when, a non-2xx response supplied a status; every other write failure (the failure classes are enumerated at the end of this section) shows the bare headline, so a status in the toast is always one the server actually returned. One status is also named: an add answered with 420 (Trakt's "Account Limit Exceeded"; observed live 2026-09-18: adding to a list at its item cap yields 420 through the app's apiz host; inferred, not observed: no other account limit applies to adding an item to an existing list, so the status is read as the per-list item cap) reads `Couldn't add "<title>" to <list>: list is full (HTTP 420)`. The cap's value is deliberately not quoted, since Trakt has lowered it before. Every other status, and a 420 on a remove (which cannot exceed a limit, so it is left unexplained), keeps the plain `(HTTP <status>)` suffix. The title comes from the captured menu context (card path: the three-dot button's `aria-label`; summary path: the page title heading, slug fallback); if no title was captured, the word `item` stands in for the quoted title (`Couldn't add item to <list>`). One further toast sits outside the write path: when the target list no longer resolves at click time (deleted or renamed between render and click), nothing is sent, nothing is applied or reverted, and the toast reads `Couldn't update <list>: list not found`, never with a status. One toast element: a failure arriving while one is showing replaces the message and restarts the dismiss timer (concurrent failures are possible since the in-flight guard is per list+item, but rare enough that stacking is not worth building). Details to the console. Two write-failure classes, same handling (reconcile refresh per Writes step 6, revert + toast per step 7): transport failures (no token so nothing sent, network error, timeout, non-2xx, or a 2xx add whose body cannot be parsed; of these only non-2xx carries a status) and item rejection (a parsed 2xx add body in which our item did not land, per Writes step 5).
...
Manual checklist (the suites under `tests/` do not cover this feature):
...
- Failure path (devtools offline): toast appears with the bare headline (no status), state reverts, and a reconciling refresh fires once back online or on next trigger.
- Failure path with a status (a non-2xx response other than 420 on an add, e.g. a devtools request override returning 500): the toast ends in `(HTTP <status>)` and carries no reason text; state reverts.
- Full list (add to a list at Trakt's item cap): the toast reads `...: list is full (HTTP 420)`; state reverts. A remove from the full list is expected to still succeed (inferred from the truncate feature removing from the capped list, not separately observed); the script judges a remove successful on any 2xx.
