...
`AGENTS.md` is the canonical instruction file. Edit it directly; keep `CLAUDE.md` as an import-only pointer.

## Commit Conventions

This repo does not use a floating host-specific commit convention. Changes to AGENTS.md and `.claude/*` files are committed as regular Conventional Commits, and may share a commit with related changes when they belong to the same logical change.
...
- An injected page-context build reaches only hosts on the app's CSP connect-src whitelist and has no GM APIs, so its stub seams must cover EVERY remote transport in the driven path (upstream id-resolution fetches included, not just the feature's own new fetch); a single unstubbed hop fails the whole chain before the surface under test runs. Exclude the scan bootstrap from injected builds (an injection-time scan fires before stubs are assigned and sends real traffic); publish the scan-trigger function (`queueScan` in this file) on the window handle alongside the surface under test, so scans can be driven explicitly instead.
- A fully hidden browser window (minimized or behind a locked screen; `document.visibilityState === 'hidden'`) goes beyond timer clamping: trusted computer-tool clicks stop opening the app's menus at all, so any e2e leg that needs a real menu or drawer interaction is blocked until the window is visible again. In-eval sleep-based poll loops also inflate under the clamp and can push the eval past the 45s CDP timeout. Check visibilityState first; when hidden, fall back to flow-level verification through the window handle against a synthetic fixture (real markup shapes, real writes through the feature's entry function, single-shot evals with cross-tool-call gaps standing in for waits), and reserve the real-surface legs for a visible window.
...
- Since the manage-lists row takeover shipped (Trakt Improved 1.35), the installed script owns the Anticipated/Uninterested rows of the Manage lists drawer: it suppresses the full event sequence on owned rows, corrects their label/fill attributes per scan, and stamps `data-mlrt-item` markers, so drawer e2e against a 1.35+ install must rename the `mlrt` marker axis (alongside keys and classes) or the two instances contest row attributes. Live re-probe 2026-08-07 also falsified two 2026-08-05 records for the current app version: the drawer UNMOUNTS on dismissal (no parked node; each open creates fresh rows), and the menus' "Manage lists" rows no longer open the drawer on synthetic click sequences (trusted clicks required); the row's text is the exact string "Manage lists" plus a trailing U+2026 ellipsis on both menu surfaces.
- The same clamping starves timer-driven scan scheduling in an injected build: a band suite that awaits queueScan's rAF/setTimeout path can exceed the tooling's 45s eval timeout, and the timed-out eval keeps executing as a zombie continuation that resumes on clamped timers and mutates shared state minutes later. Publish the scan-callbacks list (or a synchronous scan driver) on the window handle and drive scans synchronously, draining async completions with microtask chains only (a chained microtask loop also starves macrotasks, which keeps zombies from interleaving mid-eval); before re-running after a timeout, poison the zombie's captured helper methods so its next call throws. A band whose seeded state triggers a fire-and-forget resolution (e.g. an expired entry calling resolveIds) leaks that completion into the next band's microtask drains; isolate such bands or re-run the contaminated one.

## Backlogs and indexes

Four repo-local indexes live under `.claude/`. A `SessionStart` hook in `.claude/settings.json` injects a directive so coding agents read them on the first turn of every session; any task the user raises may already be queued, designed, diagnosed, or covered by an existing pattern:
